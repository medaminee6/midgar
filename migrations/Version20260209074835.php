...
final class Version20260209074835 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $commandeFkExists = (int) $this->connection->fetchOne(
            "SELECT COUNT(*) FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'FK_6EEAA67DF347EFB'"
        );
        if ($commandeFkExists === 0) {
            $this->addSql('ALTER TABLE commande ADD CONSTRAINT FK_6EEAA67DF347EFB FOREIGN KEY (produit_id) REFERENCES produit (id)');
        }

        $personnageFkExists = (int) $this->connection->fetchOne(
            "SELECT COUNT(*) FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'FK_6AEA486D5CD9AF2'"
        );
        if ($personnageFkExists === 0) {
        ...
        $reponsesFkExists = (int) $this->connection->fetchOne(
            "SELECT COUNT(*) FROM information_schema.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'FK_1E512EC61E27F6BF'"
        );
        if ($reponsesFkExists === 0) {
            $this->addSql('ALTER TABLE reponses ADD CONSTRAINT FK_1E512EC61E27F6BF FOREIGN KEY (question_id) REFERENCES questions (id)');
        }
        if ($schema->hasTable('artefacts')) {
